...
describe("dedicated Nordic Deep Research report discovery", () => {
  ...
  it("keeps the runtime-proven Main Market scope and leaves First North unsupported until separately verified", () => {
    const engine = file("lib/model-portfolios/engine/nordic-primary-sources.ts");

    assert.match(
      engine,
      /url\.searchParams\.set\("globalName",\s*"NordicMainMarkets"\)/,
    );
    assert.match(
      engine,
      /First North therefore remains an explicit unsupported source scope/,
    );
    assert.doesNotMatch(engine, /includeGrowthMarkets\?: boolean/);
    assert.doesNotMatch(
      engine,
