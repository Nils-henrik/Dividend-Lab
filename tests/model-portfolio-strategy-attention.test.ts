...
describe("model portfolio strategy attention", () => {
  ...
  it("5b. keeps a held non-income Dividend name monitorable without admitting the identical non-held name", () => {
    const held = selectPortfolioAttentionCandidates({
      universe: MIXED_UNIVERSE,
      strategyKey: "dividend",
      heldInstruments: [{ symbol: "QUALITY", exchange: "ST" }],
    });
    const fresh = select("dividend");
    const monitored = held.candidates.find((item) => item.symbol === "QUALITY");
    assert.ok(monitored);
    assert.equal(monitored.attentionEligibility, "held_for_monitoring");
    assert.ok(!newEntrySymbols(fresh.candidates).includes("QUALITY"));
    assert.ok(fresh.rejectedNewEntries.some((item) => item.symbol === "QUALITY" && item.reasons.includes("rejected_non_income")));
  });

  it("6b. keeps a held degraded Conservative instrument monitorable", () => {
    const attention = select("conservative", ["SPEC"]);
    const held = attention.candidates.find((item) => item.symbol === "SPEC");
    assert.ok(held);
    assert.equal(held.attentionEligibility, "held_for_monitoring");
  ...
  it("20. does not add historical portfolio mutation code in the prospective attention/lane layers", () => {
    const dir = path.join(path.dirname(fileURLToPath(import.meta.url)), "../lib/model-portfolios/engine");
    for (const file of ["strategy-attention.ts", "research-lanes.ts", "dry-run.ts"]) {
      const source = readFileSync(path.join(dir, file), "utf8");
      assert.doesNotMatch(source, /settleModelPortfolioDecision/);
      assert.doesNotMatch(source, /from "\.\/settle-service"/);
      assert.doesNotMatch(source, /model_portfolio_holdings/);
      assert.doesNotMatch(source, /backfill/);
    }
  });
});
