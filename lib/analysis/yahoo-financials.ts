...
function capexSigned(row: UnknownRecord | null): number | null {
  const value = firstNumber(row, ["capitalExpenditures", "capitalExpenditure"]);
...
function deriveFreeCashFlow(operatingCashFlow: number | null, capex: number | null): number | null {
  ...
  // Yahoo commonly reports capex as a negative cash-flow line. If a provider
  // returns it positive, subtract it instead. This keeps the accounting sign
  // convention explicit rather than silently assuming one representation.
...
export function parseYahooFinancialStatements(input: {
  ...
  const incomeQuarterly = historyRows(
    moduleRecord(result, "incomeStatementHistoryQuarterly"),
    "incomeStatementHistory",
  );
  const cashAnnual = historyRows(moduleRecord(result, "cashflowStatementHistory"), "cashflowStatements");
  const cashQuarterly = historyRows(
    moduleRecord(result, "cashflowStatementHistoryQuarterly"),
    "cashflowStatements",
  );
  const balanceAnnual = historyRows(moduleRecord(result, "balanceSheetHistory"), "balanceSheetStatements");
  const balanceQuarterly = historyRows(
    moduleRecord(result, "balanceSheetHistoryQuarterly"),
  ...
  const revenueTtm =
    firstNumber(financialData, ["totalRevenue"]) ??
    lastFourSum(incomeQuarterly, ["totalRevenue", "operatingRevenue"]);
  ...
  const operatingCashFlowTtm =
    firstNumber(financialData, ["operatingCashflow", "operatingCashFlow"]) ??
    lastFourSum(cashQuarterly, ["totalCashFromOperatingActivities", "operatingCashFlow"]);
  const capexTtm = lastFourSum(cashQuarterly, ["capitalExpenditures", "capitalExpenditure"]);
  const freeCashFlowTtm =
    firstNumber(financialData, ["freeCashflow", "freeCashFlow"]) ??
    deriveFreeCashFlow(operatingCashFlowTtm, capexTtm);
  ...
  const sharesOutstanding =
    positive(keyStats?.sharesOutstanding) ??
    positive(price?.sharesOutstanding) ??
    positive(firstNumber(latestBalance, [
      "ordinarySharesNumber",
      "shareIssued",
      "commonStockSharesOutstanding",
    ]));

  const previousAnnualShares = historicalPeriods.find(
    (period) => period.sharesOutstanding !== null && period.sharesOutstanding !== undefined,
  )?.sharesOutstanding ?? null;
  const sharesOutstandingGrowthYoy =
    sharesOutstanding !== null && previousAnnualShares !== null && previousAnnualShares > 0
      ? sharesOutstanding / previousAnnualShares - 1
      : null;
  ...
  const revenueGrowthYoy =
    firstNumber(financialData, ["revenueGrowth"]) ??
    yoyGrowth(latestAnnualRevenue, previousAnnualRevenue);

  const operatingMarginTtm =
    firstNumber(financialData, ["operatingMargins"]) ??
    (revenueTtm && operatingIncomeTtm !== null ? operatingIncomeTtm / revenueTtm : null);
  const profitMarginTtm =
    firstNumber(financialData, ["profitMargins"]) ??
    (revenueTtm && netIncomeTtm !== null ? netIncomeTtm / revenueTtm : null);
  const ebitdaTtm = firstNumber(financialData, ["ebitda"]);
  const epsTtm = firstNumber(keyStats, ["trailingEps"]) ??
    (netIncomeTtm !== null && sharesOutstanding ? netIncomeTtm / sharesOutstanding : null);
