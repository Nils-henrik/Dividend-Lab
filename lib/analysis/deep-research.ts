...
import {
  buildDivLabAnalysisChartModel,
  type DivLabAnalysisChartModel,
} from "./chart-model";
import {
...
export type DivLabResearchPacket = {
  ...
  /** Immutable chart payload. Historical analyses never reload later price bars. */
  chart: DivLabAnalysisChartModel;
...
export function buildDivLabResearchPacket(input: {
  ...
  const chart = buildDivLabAnalysisChartModel({
    history: input.history,
    levels,
    currentPrice: input.currentPrice,
    asOf: technicalSnapshot.asOf,
  });
