...
function clusterPivots(pivots: readonly Pivot[], absoluteTolerance: number): ZoneCluster[] {
  ...
  for (const pivot of ordered) {
    const closest = clusters
      ...
      .filter((candidate) => Math.abs(candidate.center - pivot.price) <= absoluteTolerance)
      .sort(
        (a, b) =>
...
function buildZone(input: {
  ...
  const lower = Math.min(...cluster.prices, center - absoluteTolerance * 0.35);
  const upper = Math.max(...cluster.prices, center + absoluteTolerance * 0.35);
