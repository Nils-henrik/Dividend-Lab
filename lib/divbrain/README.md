...
- **Shared (this folder today):** `constants`, `types`, `errors`, `results`, `validation`, `sources`, `citations`, `guardrails`, `attachments` — safe to import from server or client.
- **Server-only by convention:** `server/guardrails`, `server/guardrail-evals`, `server/providers`, `server/benchmark`, `server/identity`, `server/policy`, `server/context`, `server/context-evals`, `server/repository`, `server/service`, `server/access`, `server/ui`, `server/learning`, `server/attachments` — detection logic, eval fixtures, identity/policy text, context assembly, provider boundary, Phase 1B benchmark harness, conversation persistence, application-service orchestration, Internal Alpha access, the `/brain` shell loader, Learning lexical retrieval, and private chat attachments. These modules live under `lib/divbrain/server/` and **must never be imported by client components** or other browser-safe UI. Package-level `import "server-only"` is deferred until an approved dependency/foundation ticket; folder placement and this README are the current boundary.
...
- `cost-guard-config.ts` / `cost-guard.ts` / `usage-accounting.ts` — Issue #105 atomic Cost Guard + post-call accounting
- `server/benchmark/**` — deterministic multi-candidate harness + local rubric
- `server/repository/usage-ledger*` — persistent usage ledger with atomic reserve/finalize (micro-USD; service_role only)
...
- Token-usage fields are optional normalized integers; Cost Guard projects/estimates from the candidate catalog and may record validated Gateway actual cost (including decimal strings) as integer micro-USD.
- Real AI Gateway generation is blocked unless Cost Guard config + usage ledger **atomic reservation** admits the request.
