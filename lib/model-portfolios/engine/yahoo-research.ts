...
import type { EodhdFundamentalsSnapshot, ResearchFundamentalScores } from "./research-fundamentals";
...
export async function fetchYahooHistoryResearch(
  ...
  try {
    ...
    const timestamp = isoFromEpochSeconds(meta.regularMarketTime) ??
      (history.at(-1) ? `${history.at(-1)!.date}T21:00:00.000Z` : null);
    ...
    const quote: DelayedQuote | null = regularPrice && timestamp
      ? {
          ...
          changePct:
            previousClose && previousClose > 0 ? ((regularPrice / previousClose) - 1) * 100 : null,
          delayed: true,
          provider: "eodhd",
        }
      : null;
...
export async function fetchYahooFundamentals(
  ...
  try {
    ...
    const marketCap = readModuleNumber(price, "marketCap");
    const snapshot: EodhdFundamentalsSnapshot = {
    ...
    const useful = Object.values(scores).some((value) => value !== undefined && value !== null);
