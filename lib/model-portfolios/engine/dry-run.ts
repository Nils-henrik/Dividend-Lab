  ...
  return candidates
    .map((candidate, index) =>
      [
        ...
        `recoveryState=${candidate.recoverySetup.state}`,
        `recoveryScore=${candidate.recoverySetup.score}`,
        `drawdownFrom52WeekHigh=${candidate.recoverySetup.drawdownFrom52WeekHigh ?? "n/a"}`,
        `fundamentalIntegrity=${candidate.recoverySetup.fundamentalIntegrityScore}`,
        `entryConfirmation=${candidate.recoverySetup.entryConfirmationScore}`,
        `recoveryReasons=${candidate.recoverySetup.reasons.join(" / ") || "n/a"}`,
