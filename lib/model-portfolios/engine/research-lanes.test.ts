...
import {
  ...
  EVENT_PRIMARY_SOURCE_TARGET_COUNT,
...
describe("strategy research lanes", () => {
  it("keeps Nordic and US deep-research budgets numerically unchanged", () => {
    ...
    assert.equal(EVENT_PRIMARY_SOURCE_TARGET_COUNT, 2);
    assert.equal(EVENT_PRIMARY_SOURCE_TARGET_COUNT, NORDIC_RESEARCH_BOUNDS.eventPrimarySourceTargetCount);
    assert.match(pipeline, /selectCrossLaneEventTargets/);
    assert.doesNotMatch(pipeline, /const eventTargets = \[\.\.\.targets\]\.slice\(0, googleTargetLimit\)/);
  });
  ...
  it("never silently truncates holdings when they exceed the discovered US seed cap", () => {
    const holdings = Array.from({ length: 22 }, (_, index) => ({
      symbol: `HELD${index}`,
      exchange: "US",
      name: `Held ${index}`,
      yahooSymbol: `HELD${index}`,
      held: true as const,
    }));
    const movers = Array.from({ length: 20 }, (_, index) => ({
      symbol: `MOVER${index}`,
    ...
    const seeds = selectUsSharedSeedUnion({ holdings, movers, maxSeeds: 18 });
    assert.equal(seeds.filter((item) => item.held).length, 22);
    assert.equal(seeds.filter((item) => !item.held).length, 0);
    assert.equal(seeds.length, 22);
    const again = selectUsSharedSeedUnion({ holdings, movers, maxSeeds: 18 });
    assert.deepEqual(
      again.map((item) => item.symbol),
      seeds.map((item) => item.symbol),
    );
  });
...
describe("Nordic geographic repair after lane routing", () => {
  it("repairs country minimums when lane-first selection would starve one country", () => {
    const universe: NordicCandidate[] = [
      nordic("SAGA-D", "ST", "SE", 0.2),
      nordic("CORE-D", "ST", "SE", 0.19),
      nordic("FPAR-D", "ST", "SE", 0.18),
      nordic("YUBICO", "ST", "SE", 0.9),
      nordic("KIT", "OL", "NO", 0.89),
      nordic("HARVIA", "HE", "FI", 0.88),
      nordic("LAGR-B", "ST", "SE", 0.87),
      nordic("INVE-B", "ST", "SE", 0.99, "large_cap"),
      nordic("EQNR", "OL", "NO", 0.98, "large_cap"),
      nordic("NOKIA", "HE", "FI", 0.97, "large_cap"),
      nordic("VOLV-B", "ST", "SE", 0.96, "large_cap"),
      nordic("DNB", "OL", "NO", 0.95, "large_cap"),
      nordic("SAMPO", "HE", "FI", 0.94, "large_cap"),
      nordic("MOVER-SE", "ST", "SE", 0.93, "large_cap"),
      nordic("MOVER-NO", "OL", "NO", 0.92, "large_cap"),
      nordic("MOVER-FI", "HE", "FI", 0.91, "large_cap"),
      nordic("NOVO-B", "CO", "DK", 0.05, "large_cap"),
      nordic("DSV", "CO", "DK", 0.04, "large_cap"),
      nordic("AMBU-B", "CO", "DK", 0.03),
    ];
    ...
    const byCountry = { SE: 0, NO: 0, FI: 0, DK: 0 };
    for (const item of shortlist) byCountry[item.country] += 1;
    assert.ok(byCountry.DK >= 2, `DK starved after lane fill: ${JSON.stringify(byCountry)} ${shortlist.map((item) => item.symbol).join(",")}`);
    for (const country of ["SE", "NO", "FI", "DK"] as const) {
      assert.ok(byCountry[country] >= 2);
      assert.ok(byCountry[country] <= 5);
    }
    const again = selectNordicLaneShortlist(universe, {
      shortlistLimit: 14,
      perCountryMin: 2,
...
describe("cross-lane event/primary-source targets", () => {
  it("keeps the two-slot budget and avoids an income-lane monopoly when other lanes exist", () => {
    const selected = selectCrossLaneEventTargets({
      orderedFundamentalKeys: ["SAGA-D.ST", "CORE-D.ST", "JNJ.US", "MSFT.US", "YUBICO.ST"],
      seeds: [
        { symbol: "SAGA-D", exchange: "ST", researchLane: "income" },
        { symbol: "CORE-D", exchange: "ST", researchLane: "income" },
        { symbol: "JNJ", exchange: "US", researchLane: "income" },
        { symbol: "MSFT", exchange: "US", researchLane: "quality_core" },
        { symbol: "YUBICO", exchange: "ST", researchLane: "high_risk_opportunity" },
      ],
      limit: EVENT_PRIMARY_SOURCE_TARGET_COUNT,
    });
    assert.equal(EVENT_PRIMARY_SOURCE_TARGET_COUNT, 2);
    assert.equal(selected.length, 2);
    assert.equal(selected[0], "SAGA-D.ST");
    assert.notEqual(selected[1], "CORE-D.ST");
    assert.ok(["MSFT.US", "YUBICO.ST"].includes(selected[1]!));
    const again = selectCrossLaneEventTargets({
      orderedFundamentalKeys: ["SAGA-D.ST", "CORE-D.ST", "JNJ.US", "MSFT.US", "YUBICO.ST"],
      seeds: [
        { symbol: "SAGA-D", exchange: "ST", researchLane: "income" },
        { symbol: "CORE-D", exchange: "ST", researchLane: "income" },
  ...
  it("still lets holdings with an explicit event need keep both slots", () => {
    const selected = selectCrossLaneEventTargets({
      orderedFundamentalKeys: ["HELD1.US", "HELD2.US", "MSFT.US"],
      seeds: [
        { symbol: "HELD1", exchange: "US", held: true, researchLane: "income", eventReviewNeeded: true },
        { symbol: "HELD2", exchange: "US", held: true, researchLane: "income", eventReviewNeeded: true },
        { symbol: "MSFT", exchange: "US", researchLane: "quality_core" },
      ],
      limit: 2,
    });
    assert.deepEqual(selected, ["HELD1.US", "HELD2.US"]);
  });
});
