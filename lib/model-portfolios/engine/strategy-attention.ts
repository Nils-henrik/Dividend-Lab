...
export type DryRunAttentionSnapshot = {
  attention: AttentionSelectionResult;
  /** Final AI snapshot: all held-for-monitoring names, then capped new entries. */
  snapshot: AttentionCandidate[];
  newEntryCandidates: AttentionCandidate[];
  heldMonitoringCandidates: AttentionCandidate[];
};

/**
 * Assemble the dry-run / AI candidate snapshot without re-ranking holdings
 * through `rankResearchUniverse()`.
 *
 * Invariant: a current holding with available research survives into the final
 * monitoring snapshot even if it would fail as a new entry. Held status never
 * expands the new-entry deep cap (`RESEARCH_BUDGET.maxDeepResearchCandidates`).
 */
export function selectDryRunAttentionSnapshot(
  input: AttentionSelectionInput,
): DryRunAttentionSnapshot {
  const attention = selectPortfolioAttentionCandidates(input);
  const heldMonitoringCandidates = attention.candidates.filter(
    (item) => item.attentionEligibility === "held_for_monitoring",
  );
  const eligibleNewEntries = attention.candidates.filter(
    (item) => item.attentionEligibility === "new_entry",
  );
  const newEntryCandidates = eligibleNewEntries.slice(0, RESEARCH_BUDGET.maxDeepResearchCandidates);
  return {
    attention,
    snapshot: [...heldMonitoringCandidates, ...newEntryCandidates],
    newEntryCandidates,
    heldMonitoringCandidates,
  };
}

export function isFreshBuyEligible(
  candidate: Pick<AttentionCandidate, "attentionEligibility">,
  newEntryEligible: boolean,
): boolean {
  if (candidate.attentionEligibility === "new_entry") return true;
  return newEntryEligible;
}

export const MODEL_PORTFOLIO_STRATEGY_KEYS = [
  "conservative",
  "balanced",
  "high_risk",
  "dividend",
...
export type FourManagerAttentionComparison = Record<
  ModelPortfolioStrategyKey,
  {
    newEntrySymbols: string[];
    heldMonitoringSymbols: string[];
    rejectedNewEntrySymbols: string[];
  }
>;

/**
 * Non-settling deterministic shadow comparison of the four manager attention
 * sets against the same fetched pool. Does not call models or mutate history.
 */
export function compareFourManagerAttentionSets(input: {
  universe: readonly ResearchCandidate[];
  holdingsByStrategy?: Partial<
    Record<ModelPortfolioStrategyKey, readonly HeldInstrumentRef[]>
  >;
}): FourManagerAttentionComparison {
  const comparison = {} as FourManagerAttentionComparison;
  for (const strategyKey of MODEL_PORTFOLIO_STRATEGY_KEYS) {
    const snapshot = selectDryRunAttentionSnapshot({
      universe: input.universe,
      strategyKey,
      heldInstruments: input.holdingsByStrategy?.[strategyKey] ?? [],
    });
    comparison[strategyKey] = {
      newEntrySymbols: snapshot.newEntryCandidates.map((item) => item.symbol),
      heldMonitoringSymbols: snapshot.heldMonitoringCandidates.map((item) => item.symbol),
      rejectedNewEntrySymbols: snapshot.attention.rejectedNewEntries.map((item) => item.symbol),
    };
  }
  return comparison;
}
