...
describe("model portfolio simulated settlement", () => {
  ...
  it("updates holdings and cash on sell", () => {
    const plan = planSimulatedSettlement({
      ...baseBuyInput,
      side: "sell",
      targetWeightPct: 0,
...
describe("model portfolio market live status", () => {
  ...
  it("waits in red before US open on a Swedish holiday", () => {
    // Swedish Labour Day 2026-05-01; US markets are open that Friday.
    // 12:00 Stockholm = 10:00 UTC, before US open (13:30 UTC / 09:30 ET in summer).
  ...
  it("shows LIVE USA once US opens on a Swedish holiday", () => {
    // 16:00 UTC = 12:00 ET on 2026-05-01
  ...
  it("keeps Sweden live on a US holiday while XSTO is open", () => {
    // US Presidents' Day 2026-02-16; Stockholm open at 10:00 local = 09:00 UTC (CET).
  ...
  it("shows SE + USA overlap when both sessions are open", () => {
    // 15:45 Stockholm summer = 13:45 UTC; US already open.
  ...
  it("handles DST transition periods for Stockholm and New York", () => {
    // After EU DST start (2026-03-29) but before US DST start (2026-03-08 already passed in US...
    // Use 2026-03-10: US already on EDT, EU still CET until March 29.
    const beforeEuDst = resolveMarketSession("SE", new Date("2026-03-10T08:00:00.000Z"));
    // 08:00 UTC = 09:00 CET → Stockholm just opening.
