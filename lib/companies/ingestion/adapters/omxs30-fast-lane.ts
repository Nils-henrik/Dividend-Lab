...
export const EVOLUTION_DOCUMENT_ORIGIN = "https://storage.mfn.se";
...
const EVOLUTION_REPORT_PATH = /^\/[0-9a-f-]{36}\/[^/?#]+\.pdf$/i;
...
function europeanSlashDate(value: string): string | null {
...
export function parseEvolutionPressReleases(html: string): NormalizedCompanyDocument[] {
  ...
  for (const match of html.matchAll(
    /date-stamp">([^<]+)<\/p>[\s\S]{0,2500}?href="([^"]+)"[^>]*>[\s\S]{0,400}?<strong>([\s\S]*?)<\/strong>/gi,
  )) {
    const document = pressDocument({
      title: decodeHtmlText(match[3]),
      sourceUrl: normalizeSameOriginUrl(match[2], EVOLUTION_ORIGIN, EVOLUTION_PRESS_PATH),
      publishedAt: europeanSlashDate(match[1]),
...
export function parseEvolutionFinancialReports(html: string): NormalizedCompanyDocument[] {
  ...
  for (const match of html.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)) {
    const block = match[1];
    const title = decodeHtmlText(block.replace(/<a\b[\s\S]*?<\/a>/gi, " "));
    const publishedAt = europeanSlashDate(block);
    const pdf = block.match(/href="(https:\/\/storage\.mfn\.se\/[^"?#]+\.pdf)"/i)?.[1];
    const sourceUrl = pdf
      ? normalizeSameOriginUrl(pdf, EVOLUTION_DOCUMENT_ORIGIN, EVOLUTION_REPORT_PATH)
      : null;
...
export function parseEvolutionCalendar(
  ...
  const documents: NormalizedCompanyDocument[] = [];
  for (const match of html.matchAll(
    /<strong>([^<]+)<\/strong><\/p><span class="text-text-font-2">([^<]+)<\/span>/gi,
  )) {
    const document = calendarDocument({
      title: decodeHtmlText(match[2]),
      pageUrl: EVOLUTION_CALENDAR_SOURCE_URL,
