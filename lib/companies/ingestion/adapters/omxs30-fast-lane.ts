...
export const NIBE_NEWS_SOURCE_URL =
  `${NIBE_ORIGIN}/investors/pm-news-reports/news-reports-2026`;
export const NIBE_CALENDAR_SOURCE_URL =
  `${NIBE_ORIGIN}/investors/calendar-2025-2026`;
...
const NIBE_NEWS_PATH = /^\/investors\/pm-news-reports\/\d{4}---news-reports\/[^/]+$/;
const NIBE_CALENDAR_PATH = /^\/investors\/calendar-2025-2026\/[^/]+\/[^/]+$/;
const NIBE_PDF_PATH = /^\/download\/[^/]+\/[^/]+\/[^/]+\.pdf$/i;
...
export function parseNibePressReleases(html: string): NormalizedCompanyDocument[] {
  ...
  for (const match of html.matchAll(
    /<time\b[^>]*datetime="([^"]+)"[^>]*>[\s\S]{0,2500}?<h3\b[^>]*>\s*<a\b[^>]*href="([^"]+)"[^>]*>\s*<span>([\s\S]*?)<\/span>/gi,
  )) {
    ...
    const document = publishedAt && sourceUrl
      ? pressDocument({ title, sourceUrl, publishedAt, publisher: NIBE_PUBLISHER })
      : null;
    if (document) {
      documents.push(document);
    }
...
export function parseNibeReportCandidates(html: string): FastLaneReportCandidate[] {
  return parseNibePressReleases(html).flatMap((document) => {
    if (!classifyReportTitle(document.title) || !document.publishedAt) {
      return [];
    }
...
export function parseNibeReportPdf(
  html: string,
  candidate: FastLaneReportCandidate,
): NormalizedCompanyDocument | null {
  const pdf = [...html.matchAll(/<a\b[^>]*href="([^"]+\.pdf)"[^>]*>([\s\S]*?)<\/a>/gi)]
    .find((match) => /report/i.test(`${match[1]} ${decodeHtmlText(match[2])}`)
      && !/presentation|ppt/i.test(`${match[1]} ${decodeHtmlText(match[2])}`));
  const sourceUrl = pdf ? normalizeSameOriginUrl(pdf[1], NIBE_ORIGIN, NIBE_PDF_PATH) : null;
  if (!sourceUrl) {
    return null;
  }
...
export function parseNibeCalendar(html: string, now: Date): NormalizedCompanyDocument[] {
  const documents: NormalizedCompanyDocument[] = [];
  const nextReport = html.match(
    /id="h-NextReport"[\s\S]{0,900}?Interim report[\s\S]{0,700}?<h2\b[^>]*>\s*(Q[1-4]\s+20\d{2})\s*<\/h2>\s*<p class="normal">\s*(\d{2})\.(\d{2})\.\s*(20\d{2})/i,
  );
  if (nextReport) {
    const title = `Interim report ${nextReport[1]}`;
    const day = Number(nextReport[2]);
    const month = Number(nextReport[3]);
    const year = Number(nextReport[4]);
    const valid = new Date(Date.UTC(year, month - 1, day));
    const parsed = valid.toISOString();
    const sourceUrl = officialEventUrl(NIBE_CALENDAR_SOURCE_URL, parsed, title);
    if (
      sourceUrl &&
      valid.getUTCFullYear() === year &&
      valid.getUTCMonth() === month - 1 &&
      valid.getUTCDate() === day &&
      isReportPublicationTitle(title) &&
      valid.getTime() >= now.getTime()
    ) {
      documents.push({
        documentType: "report_date",
        title,
  ...
  for (const match of html.matchAll(
    /<small class="smallxgrey">([^<]+)<\/small>[\s\S]{0,500}?<a\b[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi,
  )) {
    const eventAt = englishDate(match[1]);
    const sourceUrl = normalizeSameOriginUrl(match[2], NIBE_ORIGIN, NIBE_CALENDAR_PATH);
    const title = decodeHtmlText(match[3]);
    if (!eventAt || !sourceUrl || !title || !isReportPublicationTitle(title)) {
      continue;
    }
