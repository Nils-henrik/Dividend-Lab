...
-- The function patch is intentionally idempotent because production may already
-- have received the emergency invariant guards before this migration is applied.
...
begin
  ...
  v_already_correct :=
    position('v_quantity <> trunc(v_quantity)' in v_definition) > 0
    and position('v_qty_after <> trunc(v_qty_after)' in v_definition) > 0
    and position('v_side = ''buy'' and v_fee_sek <> 0' in v_definition) > 0
    and position('v_side = ''sell'' and v_fee_sek <> 0' in v_definition) > 0;

  if not v_already_correct then
    v_patched := replace(
      v_definition,
