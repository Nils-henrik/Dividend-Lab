alter table public.divlab_peer_targets
  add constraint divlab_peer_targets_symbol_format_check
    check (
      instrument_symbol = upper(trim(instrument_symbol))
      and length(instrument_symbol) between 1 and 64
    ),
  add constraint divlab_peer_targets_exchange_format_check
    check (
      exchange = upper(trim(exchange))
      and length(exchange) between 1 and 16
    );

alter table public.divlab_peer_sets
  add constraint divlab_peer_sets_target_name_length_check
    check (length(trim(target_name)) between 1 and 200),
  add constraint divlab_peer_sets_methodology_check
    check (methodology_version = 'peer-comparison-v1');

alter table public.divlab_peer_set_sources
  add constraint divlab_peer_set_sources_key_length_check
    check (length(trim(source_key)) between 1 and 240),
  add constraint divlab_peer_set_sources_publisher_length_check
    check (length(trim(publisher)) between 1 and 200),
  add constraint divlab_peer_set_sources_url_length_check
    check (length(source_url) between 9 and 2048);

alter table public.divlab_peer_set_members
  add constraint divlab_peer_set_members_symbol_format_check
    check (
      instrument_symbol = upper(trim(instrument_symbol))
      and length(instrument_symbol) between 1 and 64
    ),
  add constraint divlab_peer_set_members_exchange_format_check
    check (
      exchange = upper(trim(exchange))
      and length(exchange) between 1 and 16
    ),
  add constraint divlab_peer_set_members_name_length_check
    check (length(trim(instrument_name)) between 1 and 200);
