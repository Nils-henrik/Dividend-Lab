...
All new Börsnyheter image assets in `public/news-demo/` must use readable, semantic filenames.

- Use lowercase letters and hyphens.
- Use a short article/series/topic description plus ISO date `YYYY-MM-DD` when the asset is date-specific.
- Preserve the asset's real file extension; do not rename a PNG to `.webp` without converting the file.
- Do not use autogenerated names such as `file_000...`, `ChatGPT Image...`, `Skärmbild...` or similar upload names.
- When a live asset is renamed, update the runtime image reference in the same change so no published article receives a broken image.
- Historical editor-upload paths may be resolved through the centralized legacy alias map in `lib/news/get-articles.ts` after the physical files have been given semantic names.
- **Permanent exception:** `public/news-demo/file_000000009cf48246883ae568fc196154.png` keeps its legacy filename because it is the canonical DivLab logo reference defined in section 1.1. Do not rename or delete it.
