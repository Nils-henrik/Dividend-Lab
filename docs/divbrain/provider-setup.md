# DivBrain provider setup (Phase 1B + Issue #103 / #105 Cost Guard)

**Status:** Provider foundation + runtime selection + hardened atomic Cost Guard / usage ledger implemented — **no production AI activation**  
**Audience:** Founder (Henrik) + engineers  
**Related:** Ticket 1B-1 / 1B-2 / 1B-4 / 1D-2 / 1D-3 / Issue #103 / Issue #105
...
| **Cost Guard** | Server-only **atomic reservation** before `provider.generate()` for AI Gateway (DB advisory lock + check/insert RPC). Missing/malformed/inconsistent config, unpriceable models, reservation persistence failure, or daily/monthly/request caps fail closed with zero network generation |
| **Usage ledger** | `public.divbrain_usage_events` migration + reserve/finalize RPCs (integer micro-USD). Hard limits use `reserved_cost_micro_usd`. RLS enabled; no anon/authenticated access; service_role SELECT + EXECUTE only (no direct INSERT/UPDATE/DELETE) |
...
The Alpha application service now calls the provider factory when no explicit server-side provider is supplied. This is **selection wiring only**: constructing the provider does not make a model request. A network generation call can occur only after the normal DivBrain lifecycle reaches the provider step **and** Cost Guard atomically reserves budget (authentication → Alpha gate → validation → guardrails → ownership/history → user-message persistence → context assembly → provider request mapping → **Cost Guard reserve** → provider → **finalize accounting** → assistant persistence).
...
4. **Apply the usage-ledger migration** (`supabase/migrations/20260808170000_create_divbrain_usage_ledger.sql`) via the normal env progression after Founder review — not from the Cursor task.
...
- Cost Guard **atomically reserves** projected cost **before** `provider.generate()` and enforces per-request, UTC-day, and UTC-calendar-month hard limits using a conservative projection (bounded current input + configured max output tokens).
- Hard-limit source of truth is `reserved_cost_micro_usd` for both reserved and finalized rows (conservative over-reservation). `accounted_cost_micro_usd` stores reconciled Gateway actual / estimate for observability and must never reduce hard-limit totals.
...
- Blocked safety prompts produce zero provider calls and zero reservations/usage-ledger rows.
- After a reserved provider attempt, finalize accounting **before** assistant-message persistence so transcript failure cannot erase spend. Finalize failure leaves the reserved charge in place (fail closed for later admission).
- Prefer validated Gateway actual cost when safely available (including canonical decimal strings such as `"0.00849"`); otherwise conservative catalog estimate; never silent zero for a paid call; never persist raw provider metadata.
- Account deletion uses `ON DELETE SET NULL` on `user_id` so historical spend aggregates are preserved while anonymizing the actor.
- Do not add production provider env vars merely to test wiring.
- Start with a small Gateway spend quota/rate limit and increase only after measured use. External Gateway spend quota remains the second-layer hard stop at activation.
