...
- **Issue #103 / #105:** dedicated `divbrain_usage_events` ledger with atomic reserve/finalize accounting landed; production provider remains unconfigured until Founder activation
...
- **Issue #103 / #105:** Cost Guard atomically reserves per-request / UTC-day / calendar-month hard limits before `provider.generate()`

### 1D-3 — Optional `divbrain_usage_events`

- Migration + repository writes (dev → preview → prod approval path)
- **Issue #103 / #105:** migration + least-privilege RLS/grants + atomic reserve/finalize RPCs implemented in repo; apply remotely only after Founder review
