...
function percent(value: number | null | undefined): string {
  if (!finite(value)) return "—";
  return `${value >= 0 ? "+" : ""}${new Intl.NumberFormat("sv-SE", {
...
function miniChart(input: {
  ...
  const closes = bars.map((bar) =>
    finite(bar.adjustedClose) && bar.adjustedClose > 0 ? bar.adjustedClose : bar.close,
  );
  const extras = [
    input.support?.lower,
    input.support?.upper,
    input.resistance?.lower,
    input.resistance?.upper,
  ...
  const zone = (value?: { lower: number; upper: number }) =>
    value
      ? {
          y: y(value.upper),
          height: Math.max(3, y(value.lower) - y(value.upper)),
        }
      : null;
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!analysis) {
    return new ImageResponse(
      <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "#080b10", color: "#f8fafc", fontSize: 54, fontWeight: 700 }}>
        DivLab Analys
      </div>,
      size,
    );
  ...
  const support = [...packet.chart.zones.supports].sort(
    (a, b) => Math.abs(a.distancePct) - Math.abs(b.distancePct),
  )[0];
  const resistance = [...packet.chart.zones.resistances].sort(
    (a, b) => Math.abs(a.distancePct) - Math.abs(b.distancePct),
  )[0];
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ display: "flex", width: 48, height: 48, borderRadius: 13, background: "#0a84ff", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 22 }}>DL</div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ fontSize: 22, fontWeight: 700 }}>DivLab Analys</div>
            <div style={{ fontSize: 14, color: "#64748b", marginTop: 3 }}>Fundamental + värdering + teknisk analys</div>
          </div>
        </div>
        <div style={{ display: "flex", border: `1px solid ${viewColor}55`, color: viewColor, borderRadius: 999, padding: "10px 18px", fontWeight: 800, fontSize: 15, letterSpacing: 1.4 }}>
          {viewLabel(draft.view)} SYN
        </div>
      </div>

      <div style={{ display: "flex", flex: 1, marginTop: 38, gap: 42 }}>
        <div style={{ display: "flex", width: 410, flexDirection: "column" }}>
          <div style={{ fontSize: 18, color: "#60a5fa", fontWeight: 700, letterSpacing: 1.2 }}>
            {packet.instrument.symbol}.{packet.instrument.exchange}
          </div>
          <div style={{ fontSize: 46, lineHeight: 1.04, fontWeight: 780, letterSpacing: -1.8, marginTop: 11 }}>
            {packet.instrument.name}
          </div>
          <div style={{ display: "flex", gap: 24, marginTop: 28 }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <div style={{ fontSize: 13, color: "#64748b" }}>ANALYSKURS</div>
              <div style={{ fontSize: 25, fontWeight: 700, marginTop: 6 }}>{price(packet.instrument.currentPrice, packet.instrument.currency)}</div>
            </div>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <div style={{ fontSize: 13, color: "#64748b" }}>BASSCENARIO</div>
              <div style={{ fontSize: 25, fontWeight: 700, marginTop: 6 }}>{price(base?.valuePerShare, packet.instrument.currency)}</div>
              <div style={{ fontSize: 14, color: finite(base?.upsideDownsidePct) && base!.upsideDownsidePct! >= 0 ? "#34d399" : "#f87171", marginTop: 2 }}>
                {percent(base?.upsideDownsidePct)}
              </div>
            </div>
          </div>
        ...
        <div style={{ display: "flex", width: 650, height: 330, flexDirection: "column", marginTop: 5 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 14, fontSize: 13, color: "#64748b" }}>
            <span>Analysdata fryst vid publicering</span>
            <span>divlab.se</span>
          </div>
        </div>
      </div>
