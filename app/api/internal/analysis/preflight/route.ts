...
export async function GET(request: Request) {
  ...
  if (!symbol || symbol.length > 32 || !exchange || exchange.length > 8) {
    return noStore(NextResponse.json({ status: "invalid_target", supported: false, message: "Välj en giltig nordisk aktie innan metodiken verifieras." }, { status: 400 }));
  }
  ...
  if (!resolved) {
    return noStore(NextResponse.json({
      ...
      supported: false,
      message: "Instrumentet kunde inte verifieras som en nordisk aktie.",
    }, { status: 404 }));
  }
  ...
  if (!preflight) {
    return noStore(NextResponse.json({
      ...
      supported: false,
      message: "DivLab kunde inte verifiera bolagstyp och fundamental metodik just nu. Ingen analys startas.",
    }, { status: 503 }));
  }

  const analysisEngine = analysisEngineForCompanyType(preflight.classification.type);
  const supported = analysisEngine !== null;
  return noStore(NextResponse.json({
    status: "ok",
    supported,
    target: resolved,
