...
async function main() {
  ...
  emit("target", { ok: true, symbol: resolved.symbol, exchange: resolved.exchange, yahooSymbol: resolved.yahooSymbol });
  ...
  const packet = buildUsResearchCoverageFactsPacket({ research: loaded.value, evidenceBundle: extraction.bundle });
  const coverage = evaluateUsResearchCoverage({ packet, evidenceQualityGate: extraction.bundle.qualityGate });
  emit("coverage", {
  ...
  if (!coverage.ready || coverage.score !== 100) process.exitCode = 6;
}
