/**
 * Compose DivLab Börsnyheter cover for Norden i centrum, 7 Augusti 2026.
 *
 * Scenery/panel backgrounds are generated assets. Masthead, headline, date,
 * Nordic flags, company names and captions are applied deterministically via
 * SVG + sharp (no AI-generated logos or final typography).
 *
 * Usage: node scripts/compose-norden-i-centrum-7-augusti-cover.mjs
 */
...
const outputPath = join(
  root,
  "public/news-demo/norden-i-centrum-7-augusti-2026.webp",
);
...
const PANELS = [
  {
    name: "EQL Pharma",
    caption: "EQL Pharma kämpar med marginalerna",
    file: "norden7-panel-eql.png",
  },
  {
    name: "Safello",
    caption: "Safello testas av kryptomarknaden",
    file: "norden7-panel-safello.png",
  },
  {
    name: "Equinor",
    caption: "Equinor stöds av oljerusningen",
    file: "norden7-panel-equinor.png",
  },
  {
    name: "Tokmanni",
    caption: "Tokmanni ska visa om marginalerna lättar",
    file: "norden7-panel-tokmanni.png",
  },
  {
    name: "Suominen",
    caption: "Suominen jagar 10-procentsmålet",
    file: "norden7-panel-suominen.png",
  },
  {
    name: "Novo Nordisk",
    caption: "Novo Nordisk mäts mot Lilly",
    file: "norden7-panel-novo.png",
  },
];
...
function overlaySvg(panelMeta) {
  ...
  const panelsMarkup = panelMeta
    .map((panel, index) => {
      ...
      const nameSize = panel.name.length > 18 ? 10.5 : panel.name.length > 12 ? 12 : 13.5;
      const captionLines = wrapCaption(panel.caption, panel.name.length > 16 ? 15 : 16);
      const captionStartY = PANEL_TOP + PANEL_H - 58 - (captionLines.length - 1) * 11;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}">
  ...
  <text x="${WIDTH / 2}" y="${HEADER_H + 47}" text-anchor="middle" fill="${BLUE}"
    font-family="${SANS}" font-size="12" font-weight="700" letter-spacing="2.4">7 AUGUSTI</text>
...
async function main() {
  ...
  const skyline = await sharp(join(assetsDir, "norden7-skyline.png"))
    .resize(WIDTH, HEADER_H + 40, { fit: "cover", position: "centre" })
