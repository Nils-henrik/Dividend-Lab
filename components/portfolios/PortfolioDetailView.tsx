    ...
    <div className="mx-auto w-full max-w-[1380px] space-y-6 pb-8">
      ...
      <section className={`border px-5 py-6 sm:px-7 ${accent}`}>
        ...
        <div className="mt-7 grid gap-4 sm:grid-cols-3">
          ...
          <Metric label="Månadsspar" value={`${formatSek(detail.monthlyContributionMinor)} den ${detail.contributionDay}:e`} />
          <Metric label="Senaste AI-beslut" value={detail.latestDecision ? formatDate(detail.latestDecision.createdAt) : "Inget beslut ännu"} />
        </div>
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <div>
              ...
              <ul className="mt-3 space-y-3 text-sm leading-6 text-divlab-text-secondary">
                {mandate.behavior.map((item) => (
                  <li key={item} className="flex gap-3"><span className="text-emerald-400">✓</span><span>{item}</span></li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul className="mt-3 space-y-3 text-sm leading-6 text-divlab-text-secondary">
                {mandate.explicitDoNot.map((item) => (
                  <li key={item} className="flex gap-3"><span className="text-red-400">×</span><span>{item}</span></li>
                ))}
              </ul>
            </div>
          </div>
        ...
        <aside className="border divlab-border-neutral bg-divlab-surface/45 px-5 py-6">
          ...
          <dl className="mt-4 space-y-4 text-xs">
            ...
            <PolicyRow label="Köp/ersätt-tröskel" value={`${Math.round(turnover.replacementThresholdScore * 100)} / 100`} />
      ...
      {detail.latestDecision ? (
        <section className="border divlab-border-neutral bg-divlab-surface/45 px-5 py-5 sm:px-7">
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-divlab-text-muted">Senaste beslut</p>
          <p className="mt-2 text-sm leading-6 text-divlab-text-secondary">{detail.latestDecision.rationale}</p>
        </section>
      ) : null}
      ...
      <section className="border divlab-border-neutral bg-divlab-surface/45">
        <div className="flex flex-wrap items-end justify-between gap-4 px-5 pb-3 pt-5 sm:px-7">
          <div>
            ...
            <p className="mt-1 text-xs text-divlab-text-muted">25 affärer per sida. Varje rad går att öppna för fullständigt beslutsunderlag.</p>
          </div>
          <p className="text-xs text-divlab-text-muted">Sida {detail.page} av {detail.pageCount}</p>
        </div>
        ...
        <div className="flex items-center justify-between border-t divlab-border-neutral px-5 py-4 sm:px-7">
          {detail.page > 1 ? (
            <Link href={`/portfolios/${detail.slug}?page=${detail.page - 1}`} className="border divlab-border-neutral px-3 py-2 text-xs font-semibold text-divlab-text hover:border-divlab-blue/50 hover:text-divlab-blue">← Föregående</Link>
          ) : <span />}
          {detail.page < detail.pageCount ? (
            <Link href={`/portfolios/${detail.slug}?page=${detail.page + 1}`} className="border divlab-border-neutral px-3 py-2 text-xs font-semibold text-divlab-text hover:border-divlab-blue/50 hover:text-divlab-blue">Nästa →</Link>
          ) : <span />}
        </div>
      </section>
    </div>
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div><p className="text-[10px] uppercase tracking-[0.14em] text-divlab-text-muted">{label}</p><p className="mt-1 text-base font-semibold text-divlab-text">{value}</p></div>;
}

function PolicyRow({ label, value }: { label: string; value: string }) {
  return <div className="flex items-start justify-between gap-4"><dt className="text-divlab-text-muted">{label}</dt><dd className="text-right font-semibold text-divlab-text">{value}</dd></div>;
}

function Cell({ href, children, className = "" }: { href: string; children: React.ReactNode; className?: string }) {
  return <td className={`p-0 text-divlab-text-secondary ${className}`}><Link href={href} className="block px-0 py-3 pr-4">{children}</Link></td>;
}
