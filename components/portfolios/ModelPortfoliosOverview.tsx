  ...
  return (
    <div className="mx-auto w-full max-w-[1560px] space-y-5 pb-4">
        ...
        <div id="historik" className="border divlab-border-neutral bg-divlab-surface/45">
          <div className="flex items-start justify-between gap-4 px-5 pb-3 pt-5">
            <div>
              ...
              <p className="mt-1 text-xs text-divlab-text-muted">Transparens i varje genomförd affär.</p>
            </div>
            <a
              href="#historik"
              className="border divlab-border-neutral px-3.5 py-2 text-xs font-semibold text-divlab-text transition hover:border-divlab-blue/50 hover:text-divlab-blue"
            >
              Visa alla affärer
            </a>
          </div>
                ...
                {recentTransactions.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={8} className="py-12 text-center text-sm text-divlab-text-muted">
                      Inga affärer har genomförts ännu. Historiken fylls automatiskt och kan inte
                      redigeras från den här vyn när AI-förvaltningen aktiveras.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <section className="flex items-start gap-3 border border-divlab-blue/20 bg-divlab-blue/[0.08] px-4 py-3 text-xs leading-5 text-divlab-text-secondary">
        ...
        <p>
          Modellportföljerna uppdateras minst 1 gång per handelsdag. Vid materiella händelser kan AI:n
          göra riktade omprövningar, med en hård gräns på totalt 4 beslutskörningar per portfölj och dag.
          Alla genomförda affärer sparas i historiken.
        </p>
      </section>
    </div>
  );
...
function PortfolioCard({ portfolio }: { portfolio: ModelPortfolioOverview }) {
  ...
  return (
    <article className={`relative overflow-hidden border divlab-border-neutral bg-divlab-surface/55 ${style.soft}`}>
      ...
      <div className="px-5 pb-4 pt-5">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-semibold tracking-[-0.03em] text-divlab-text">{portfolio.name}</h2>
          <span className={`ml-auto border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.12em] ${style.badge}`}>
            {portfolio.riskLabel}
          </span>
        </div>
        ...
        <div className="mt-7 flex items-end justify-between gap-4">
          <div>
            <p className="text-[31px] font-semibold leading-none tracking-[-0.04em] tabular-nums text-divlab-text">
        ...
        <dl className="mt-7 divide-y divlab-border-neutral text-xs">
          ...
          <MetricRow
            ...
            value={portfolio.latestDecision ? formatDate(portfolio.latestDecision.createdAt) : statusLabel(portfolio.status)}
          />
        </dl>

        <div className="mt-5 border-t divlab-border-neutral pt-4">
          <p className="line-clamp-2 min-h-10 text-xs leading-5 text-divlab-text-secondary">
            {portfolio.latestDecision?.rationale ?? portfolio.objective}
          </p>
        </div>

        <form action={setModelPortfolioFollowAction} className="mt-4">
          <input type="hidden" name="portfolioId" value={portfolio.id} />
          <input type="hidden" name="follow" value={portfolio.isFollowing ? "false" : "true"} />
          <button
            type="submit"
            className={`w-full border py-2.5 text-xs font-semibold transition ${style.border} ${style.accent} hover:bg-white/[0.03]`}
          >
...
function TransactionRow({ transaction }: { transaction: ModelPortfolioTransaction }) {
  ...
  return (
    <tr className="border-b divlab-border-neutral text-divlab-text-secondary last:border-b-0">
      <td className="whitespace-nowrap py-3 pr-4 text-divlab-text">{formatDate(transaction.executedAt)}</td>
      <td className={`whitespace-nowrap py-3 pr-4 font-semibold ${style.accent}`}>{transaction.portfolioName}</td>
      <td className={`whitespace-nowrap py-3 pr-4 font-semibold ${typeClass}`}>{transactionLabel(transaction.transactionType)}</td>
      <td className="py-3 pr-4 text-divlab-text">
        {transaction.instrumentName} <span className="text-divlab-text-muted">({transaction.instrumentSymbol})</span>
      </td>
      <td className="whitespace-nowrap py-3 pr-4">{formatSek(transaction.grossAmountMinor)}</td>
      <td className="whitespace-nowrap py-3 pr-4">{formatNumber(transaction.quantity)}</td>
      <td className="whitespace-nowrap py-3 pr-4">
        {transaction.priceMinor === null ? "—" : `${formatSek(transaction.priceMinor)} / st`}
      </td>
      <td className="max-w-[280px] truncate py-3 pr-4" title={transaction.rationale}>{transaction.rationale}</td>
    </tr>
  );
}
...
function ProcessStep({ number, title, text }: { number: string; title: string; text: string }) {
  return (
    <div className="grid grid-cols-[30px_1fr] gap-3">
      <div className="flex h-7 w-7 items-center justify-center border border-divlab-blue/35 bg-divlab-blue/10 text-xs font-semibold text-divlab-blue">
        {number}
      </div>
      <div>
...
function MiniSparkline({ accent, flat }: { accent: string; flat: boolean }) {
  return (
    <svg aria-hidden="true" viewBox="0 0 112 44" className={`h-11 w-28 ${accent}`} fill="none">
      <path
        d={flat ? "M4 25 L108 25" : "M4 34 L16 28 L25 30 L35 22 L45 27 L57 14 L67 17 L78 10 L89 12 L99 7 L108 12"}
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinecap="round"
...
function PortfolioIcon({ type }: { type: "shield" | "balance" | "rocket" | "diamond" }) {
  if (type === "balance") {
    return <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 3v18M5 6h14M7 6l-4 7h8L7 6Zm10 0-4 7h8l-4-7ZM7 17h10" /></svg>;
  }
  if (type === "rocket") {
    return <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M14 4c3-2 6-1 6-1s1 3-1 6l-6 6-4-4 5-7Z"/><path d="m9 11-4 1-2 3 5 1m5-1 1 5-3 2-1-5"/><circle cx="15.5" cy="7.5" r="1.5" /></svg>;
  }
  if (type === "diamond") {
    return <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="m3 8 4-5h10l4 5-9 13L3 8Z"/><path d="M3 8h18M7 3l5 18 5-18"/></svg>;
  }
