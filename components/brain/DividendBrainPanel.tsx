    ...
    >
      <div className="mb-6 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-3 text-sm leading-6 text-gray-400">
            Educational observations based on your mock portfolio. No financial
            advice.
          </p>
        </div>
      ...
      <div className="mt-5 rounded-xl border border-[#D4AF37]/20 bg-[#D4AF37]/5 p-4">
        <div className="mb-3 flex items-center justify-between text-sm">
          <span className="font-medium text-white">Annual goal progress</span>
      ...
      <label className="mt-5 block">
        <span className="sr-only">Ask Dividend Brain</span>
        <textarea
          onFocus={() => setIsExpanded(true)}
          onBlur={(event) => {
            if (!event.currentTarget.value) setIsExpanded(false);
          }}
          placeholder="Ask Dividend Brain..."
          rows={isExpanded ? 5 : 2}
