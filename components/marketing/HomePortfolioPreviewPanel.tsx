    ...
    <section aria-labelledby="homepage-portfolios-heading">
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        {result.portfolios.map((portfolio) => {
          ...
          return (
            <Link key={portfolio.id} href={`/portfolios/${portfolio.slug}`} className={`group rounded-xl border divlab-border-neutral p-4 transition hover:border-white/25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-divlab-blue/40 ${style.soft}`}>
              <div className="flex items-center gap-2.5">
                <span aria-hidden="true" className={`inline-flex h-8 w-8 items-center justify-center rounded-lg border divlab-border-neutral bg-white/[0.035] ${style.accent}`}><AppIcon name={style.icon} className="h-4 w-4" /></span>
                <h3 className="truncate text-sm font-semibold text-divlab-text group-hover:text-white">{portfolio.name}</h3>
                <span className={`ml-auto rounded-md border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.1em] ${style.badge}`}>{portfolio.riskLabel}</span>
              </div>
              <div className="mt-4 flex items-end justify-between gap-3">
                <div><p className="text-[10px] uppercase tracking-[0.12em] text-divlab-text-muted">Portföljvärde</p><p className="mt-1 text-lg font-semibold tabular-nums text-divlab-text">{formatSek(portfolio.totalValueMinor)}</p></div>
                <p className={`text-sm font-semibold tabular-nums ${positive ? "text-emerald-400" : "text-red-400"}`}>{positive ? "+" : ""}{portfolio.performancePct.toFixed(2).replace(".", ",")}%</p>
              </div>
              <svg aria-hidden="true" viewBox="0 0 112 44" className={`mt-2 h-8 w-full ${style.accent}`} fill="none" preserveAspectRatio="none"><polyline points={buildSparkline(portfolio)} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" /></svg>
              <div className="mt-3 border-t divlab-border-neutral pt-3"><p className="text-[10px] uppercase tracking-[0.12em] text-divlab-text-muted">Senaste AI-beslut</p><p className="mt-1 truncate text-xs font-semibold text-divlab-text-secondary">{portfolio.latestDecision ? decisionLabel(portfolio.latestDecision.type) : "Inget beslut ännu"}</p></div>
            </Link>
          );
        })}
      </div>

      <Link href="/register" className="mt-4 flex min-h-11 items-center justify-center text-sm font-semibold text-divlab-blue transition hover:text-divlab-blue-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-divlab-blue/40">Följ portföljerna med ett gratis konto →</Link>
    </section>
  );
}
